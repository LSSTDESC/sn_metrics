...
class SNNSNMetric(BaseMetric):
    ...
    def __init__(self, lc_reference,
        ...
        self.rateSN = SN_Rate(min_rf_phase=self.min_rf_phase_qual, max_rf_phase=self.max_rf_phase_qual)
    ...
    def run(self, dataSlice,  slicePoint=None):
        ...
        for season in self.info_season['season']:
            ...
            for i in range(nslice):
                ...
                for z in zRange[i*nperSlice:(i+1)*nperSlice]:
                    ...
                    for mydaymax in daymaxRange:
                        r.append((z,mydaymax,self.min_rf_phase,self.max_rf_phase))
                gen_par = np.rec.fromrecords(r,names=['z','daymax','min_rf_phase','max_rf_phase'])
            
                if self.verbose:
                    print('NSN to simulate:',len(gen_par),time.time()-time_ref)
            ...
            if self.verbose:
                print('#### SEASON processed',time.time()-time_refb,season,np.unique(sn_tot['season']),pixRa,pixDec)

            #print(r_durz)
            duration_z = np.rec.fromrecords(r_durz, names=['z','season_length'])
            durinterp_z = interp1d(duration_z['z'],duration_z['season_length'],bounds_error=False,fill_value=0.)
    ...
    def effi(self,sn_tot):
        ...
        for key in x1_color:
            ...
            if self.ploteffi:
                ax.errorbar(effi['z'],effi['effi'],yerr=effi['effi_err'],marker='o')
    ...
    def zlim(self,effi_tot,rateInterp):
        ...
        for key in x1_color:
            ...
            effiInterp = interp1d(effi['z'],effi['effi'],kind='linear',bounds_error=False,fill_value=0.)
            ...
            if nsn_cum[-1]<= 1.e-5:
                ...
                r.append(zlim(0.95))
            names.append('zlim_{}'.format(self.nameSN[(np.round(key[0],1),np.round(key[1],1))]))
